...
-- I use `const`` here to extract fist param from function - Accumulator of foldLeft
length ::
  List a
  -> Int
...
filter ::
  ...
  -> List a
filter cond =
  foldRight (\a acc -> if cond a then a :. acc else acc) Nil
...
find ::
  ...
  -> Optional a
find cond =
  foldRight (\a -> if cond a then const . Full $ a else id) Empty
