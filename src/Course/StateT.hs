...
instance Functor k => Functor (StateT s k) where
  (<$>) ::
    ...
    -> StateT s k b
  f <$> sa =
   StateT (\s -> first f <$> runStateT sa s)
   -- the same
   -- StateT (\s -> (\(a, s') -> (f a, s')) <$> runStateT sa s)
...
distinct' ::
  ...
  -> List a
distinct' xa =
  eval' (filtering f xa) S.empty
  where
    f a = StateT (\s -> pure (S.notMember a s, S.insert a s))
...
instance Monad k => Applicative (OptionalT k) where
  ...
  OptionalT f <*> OptionalT a =
    ...
    -- or
    -- OptionalT ( f >>= (\f' -> applyOptional f' <$> a) )
...
instance Monad k => Monad (OptionalT k) where
  (=<<) ::
    ...
    -> OptionalT k b
  f =<< OptionalT ka =
    OptionalT ( onFull (runOptionalT . f) =<< ka)
...
distinctG xa =
  runOptionalT (evalT (filtering f xa) S.empty)
  where
    f a = StateT (\s ->
                    OptionalT ( if a > 100
