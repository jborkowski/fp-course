...
findM _ Nil =
  pure Empty
findM p (h :. t) =
  p h >>= (\r -> if r then pure (Full h) else findM p t)
...
isHappy ::
  ...
  -> Bool
isHappy =
  contains 1 . firstRepeat . produce (toInteger . sum . map squareDigits . show')
  where
    squareDigits = square . digitToInt
    square = join (*)
